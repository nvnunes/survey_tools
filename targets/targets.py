...
def load_details(experiment, id=None, index=None):
    output_path = _get_output_path()
...
def plot_catalog_spectra(galaxy, catalog_ids, figsize=None, lines=None, options=None, show_emission_lines=False, show_skylines=False):
    num_plots = 0
    ...
    for catalog_name in catalog.get_spectra_sources():
        if catalog_name in catalog_ids['catalog']:
            ...
            spectra = catalog.get_spectra(catalog_name, source_id, galaxy['ra'], galaxy['dec'])
...
def _plot_spectra(galaxy, spectrum, figsize=None, options=None, lines=None, show_emission_lines=False, show_skylines=False):
    ...
    if lines is None:
        not_nan_filter = ~np.isnan(flux_1d)
        min_wavelength = wavelength[np.where(not_nan_filter)[0][0]]
        max_wavelength = wavelength[np.where(not_nan_filter)[0][-1]]
    ...
    if table.has_field(spectrum, 'source_name') and table.has_field(spectrum, 'source_id'):
        if f"{spectrum.source_id:.0f}" in spectrum.source_name:
            title = f"{spectrum.catalog} {spectrum.source_name}"
        else:
            title = f"{spectrum.catalog} {spectrum.source_name} ({spectrum.source_id:.0f})"
    elif table.has_field(spectrum, 'source_name'):
        title = f"{spectrum.catalog} {spectrum.source_name}"
    elif table.has_field(spectrum, 'source_id'):
        title = f"{spectrum.catalog} {spectrum.source_id:.0f}"
    else:
        title = spectrum.catalog
